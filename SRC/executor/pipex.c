...
int	call(t_cmd *cmd)
{
	...
	if (!pid)
	{
		...
		close(cmd->fd_in[0]);
		path = get_path(cmd->args[0], cmd->env); //env
		if (!path)
			exit_val = 1;
//			exit_val = ppx_patherr(oargv, envp, path, argv);
		else if (!path || 0 > dup2(cmd->prev_pipe[0], 0) || 0 > dup2(cmd->fd_in[1], 1)) //else if (!path || 0 > dup2(fds[0], 0) || 0 > dup2(fds[1], 1)) 
			exit_val = errno;
		else if (execve(path, cmd->args, cmd->env) == -1) //env
			exit_val = errno;
		if (path)
			free(path);
		exit(exit_val);
